...
module.exports = postcss.plugin(
  'postcss-uniapp-tailwindcss-compatible',
  (opts = {}) => {
    opts = { ...config, ...opts };
    ...
    const isUnitIgnoreSelector = opts.unitIgnore
      && Array.isArray(opts.unitIgnore.selector)
      && opts.unitIgnore.selector.length > 0;
    const isUnitIgnoreProp = opts.unitIgnore
      && Array.isArray(opts.unitIgnore.prop)
      && opts.unitIgnore.prop.length > 0;
    const isUnitIgnoreValue = opts.unitIgnore
      && Array.isArray(opts.unitIgnore.value)
      && opts.unitIgnore.value.length > 0;
    const isSelectorRules = opts.selectorRules
      && Array.isArray(opts.selectorRules)
      && opts.selectorRules.length > 0;
    // Work with options here
    return (root) => {
      if (!isSelectorRules) return;
          ...
          covertRules.forEach((item) => {
            ...
            if (isUnitIgnoreSelector) {
              ...
              const isIgnoreSelector = opts.unitIgnore.selector.some((item) => (
                (typeof item === 'string' && item === selector)
                  || (isRegExp(item) && item.test(selector))
              ));
              if (isIgnoreSelector) {
                return;
              }
            ...
            if (isUnitIgnoreProp) {
              const isIgnoreProp = opts.unitIgnore.prop.some((item) => (
                (typeof item === 'string' && item === decl.prop)
                  || (isRegExp(item) && item.test(decl.prop))
              ));
              if (isIgnoreProp) {
                return;
              }
              ...
              match.forEach((matchItem) => {
                ...
                if (isUnitIgnoreValue) {
                  const isIgnoreValue = opts.unitIgnore.value.some((item) => (
                    (typeof item === 'string'
                        && item.toLowerCase() === val)
                      || (isRegExp(item) && item.test(val))
                  ));
                  if (isIgnoreValue) {
                    return;
                  }
